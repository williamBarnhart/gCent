...
%     quiver((X(1:n)-xref)/1e3,(Y(1:n)-yref)/1e3,data(1:n),data([1:n]+n))
    quiver((X(odds)),(Y(odds)),data(odds),data(evens),1);
    hold on
    quiver((X(odds)),(Y(odds)),synth(odds)',synth(evens)',1,'r');
    patch([out.xfault],[out.yfault],[out.zfault], slip(1:Npatch)');
    zrev
    axis image

    subplot(1,2,2)
% %     quiver((X(1:n)-xref)/1e3,(Y(1:n)-yref)/1e3,synth(1:n)',synth(n+[1:n])');
%     quiver((X(odds)-xref)/1e3,(Y(odds)-yref)/1e3,synth(odds)',synth(evens)',1);
%     axis image
%     title(['smoothing=' num2str(smoo)])

%     subplot(2,2,3)
%     quiver((X(1:n)-xref)/1e3,(Y(1:n)-yref)/1e3,res(1:n)',res([1:n]+n)');
    quiver((X(odds)),(Y(odds)),res(odds)',res(evens)',1);
    axis image
     hold on
    patch([out.xfault],[out.yfault],[out.zfault], slip(1:Npatch)');
    zrev
...
%     patch((boxx-xref)/1e3,(boxy-yref)/1e3,res(1:np)');
    quiver((X(odds)-xref)/1e3,(Y(odds)-yref)/1e3,res(odds)',res(evens)',1);
    axis image
    title('new res')
    
    subplot(2,2,2)
%     patch((boxx-xref)/1e3,(boxy-yref)/1e3,oldres(1:np)');
    quiver((X(odds)-xref)/1e3,(Y(odds)-yref)/1e3,oldres(odds)',oldres(evens)',1);
    axis image
    
    title('old res')
    subplot(2,2,3)
%     patch((boxx-xref)/1e3,(boxy-yref)/1e3,oldres(1:np)'-res(1:np)');
    quiver((X(odds)-xref)/1e3,(Y(odds)-yref)/1e3,oldres(odds)'-res(odds)',oldres(evens)'-res(evens)',1);
    axis image
