...
if(old)
    ...
    oldmod = models(:,id);
    [oldres,oldslip,oldresnorm,out,oldsynth]=forward_EQ_jRi(models(:,id),xytype,resampstruct);
...
if(startcount==0)
    ...
    for j=1:ns
        tic
        [res,slip,jRi] = forward_EQ_jRi(models(:,j),xytype,resampstruct);
        misfit(j)  = jRi;
        update_time
        EQt=EQt+tmp;
    end
...
for i=1:niter
  ...
  for j=1:nr*nn
    tic
    [res,jnk,jRi]            = forward_EQ_jRi(allmod(:,j),xytype,resampstruct);
    misfit(end+1)  = jRi;
    update_time
    EQt=EQt+tmp;
  end
